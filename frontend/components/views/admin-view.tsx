...
export function AdminView() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/job-runs?id=${id}`, { method: "DELETE" })
      if (response.ok) {
        fetchJobRuns()
      }
    } catch (error) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="border-b border-border bg-card px-4 lg:px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl lg:text-2xl font-semibold text-foreground">Admin</h1>
            <p className="text-sm text-muted-foreground mt-1">Job execution audit and monitoring</p>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Select value={jobTypeFilter} onValueChange={setJobTypeFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="option_generation">Option Generation</SelectItem>
              <SelectItem value="weekly_learning">Weekly Learning</SelectItem>
              <SelectItem value="metrics_collection">Metrics Collection</SelectItem>
            </SelectContent>
          </Select>
          ...
          <Button variant="outline" onClick={fetchJobRuns} className="sm:ml-auto">
            Refresh
          </Button>
        </div>
      </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                        ...
                        {jobRun.duration_seconds && (
                          <div className="flex items-center gap-1">
                            ...
                            <span>{formatDuration(jobRun.duration_seconds)}</span>
                          </div>
                        )}
                        {jobRun.topics_ingested !== undefined && (
                          <div className="flex items-center gap-1">
                            ...
                            <span>
                              {jobRun.topics_ingested} ingested, {jobRun.topics_saved || 0} saved
                            </span>
                          </div>
                        )}
                      </div>
                      ...
                      {jobRun.error_message && (
                        <div className="mt-2 flex items-start gap-2 text-sm text-destructive">
                          ...
                          <span className="line-clamp-1">{jobRun.error_message}</span>
                        </div>
                      )}
                    </div>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedJobRun && (
                <>
                  {selectedJobRun.job_type.replace(/_/g, " ")} • {selectedJobRun.id}
                </>
              )}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="mt-1">{getStatusBadge(selectedJobRun.status)}</div>
                </div>
                <div>
                  ...
                  <div className="mt-1 grid grid-cols-3 gap-4">
                    {selectedJobRun.topics_ingested !== undefined && (
                      <div>
                        ...
                        <div className="text-lg font-semibold">{selectedJobRun.topics_ingested}</div>
                      </div>
                    )}
                    {selectedJobRun.topics_saved !== undefined && (
                      <div>
                        <div className="text-xs text-muted-foreground">Saved</div>
                        <div className="text-lg font-semibold">{selectedJobRun.topics_saved}</div>
                      </div>
                    )}
                    {selectedJobRun.topics_processed !== undefined && (
                      <div>
                        <div className="text-xs text-muted-foreground">Processed</div>
                        <div className="text-lg font-semibold">{selectedJobRun.topics_processed}</div>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {selectedJobRun.metadata && Object.keys(selectedJobRun.metadata).length > 0 && (
                <div>
